    ...
    <div>
      ...
      <section className="bg-charcoal text-cream">
        <div className="max-w-7xl mx-auto px-5 md:px-8 py-14 md:py-16">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-0 md:divide-x md:divide-cream/10">
            {[
              { value: '47', label: T.stats.rescued, sub: T.stats.since },
              { value: '12', label: T.stats.inCare, sub: T.stats.inCareAt },
              { value: '31', label: T.stats.rehomed, sub: T.stats.rehomedTo },
              { value: '94%', label: T.stats.recovery, sub: T.stats.recoveryOf },
            ].map((stat) => (
      ...
      <section className="max-w-7xl mx-auto px-5 md:px-8 py-20 md:py-28">
        <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
          <div>
            <p className="text-xs font-semibold tracking-widest uppercase text-green mb-4">{T.mission.label}</p>
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-5 -left-5 bg-green text-cream px-5 py-4 rounded-sm hidden md:block">
              <div className="font-serif text-2xl">{T.mission.years}</div>
              <div className="text-xs text-cream/70 mt-0.5">{T.mission.yearsOf}</div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
            <div>
              ...
              <h2 className="font-serif text-4xl md:text-5xl text-charcoal">{T.horses.heading}</h2>
            </div>
          ...
          {/* 2 cols on mobile, 6 cols on desktop */}
          <div className="grid grid-cols-2 md:grid-cols-6 gap-3 md:gap-4">
            {horses.map((horse) => (
              <HorseCard key={horse.id} horse={horse} navigate={navigate} T={T} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="help" className="py-20 md:py-28 scroll-mt-16">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="max-w-xl mb-14">
            <p className="text-xs font-semibold tracking-widest uppercase text-green mb-3">{T.help.label}</p>
            <h2 className="font-serif text-4xl md:text-5xl text-charcoal mb-4">{T.help.heading}</h2>
            <p className="text-charcoal/60 leading-relaxed">{T.help.sub}</p>
          </div>
      ...
      <section className="bg-charcoal py-20 md:py-28 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="grid md:grid-cols-2 gap-12 md:gap-20 items-center">
            <div>
              <p className="text-xs font-semibold tracking-widest uppercase text-sky mb-4">{T.campaign.label}</p>
              <h2 className="font-serif text-4xl md:text-5xl text-cream mb-5 leading-tight">
                {activeCampaign.title}
              </h2>
              <p className="text-cream/60 leading-relaxed mb-8">{activeCampaign.description}</p>

              <div className="mb-8">
                <div className="flex justify-between text-sm mb-2.5">
                  <span className="text-cream font-semibold">€{activeCampaign.raised.toLocaleString()} {T.campaign.raised}</span>
                  <span className="text-cream/40">{T.campaign.goal}: €{activeCampaign.target.toLocaleString()}</span>
                </div>
                ...
                <div className="mt-2 text-right text-xs text-cream/35">{pct}{T.campaign.pctOfGoal}</div>
              </div>
        ...
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex items-start gap-5">
              <div className="mt-1 text-green shrink-0"><IconDoc /></div>
              <div>
                <h3 className="font-serif text-2xl text-charcoal mb-2">{T.transparency.heading}</h3>
                <p className="text-charcoal/60 text-sm leading-relaxed max-w-lg">{T.transparency.body}</p>
              </div>
            </div>
            <button
              onClick={() => navigate('donations')}
      ...
      <section className="py-20 md:py-28">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="mb-12">
            <p className="text-xs font-semibold tracking-widest uppercase text-green mb-3">{T.stories.label}</p>
            <h2 className="font-serif text-4xl text-charcoal">{T.stories.heading}</h2>
          </div>
      ...
      <section id="contact" className="bg-cream-dark py-20 md:py-28 scroll-mt-16">
        <div className="max-w-7xl mx-auto px-5 md:px-8">
          <div className="grid md:grid-cols-2 gap-12 md:gap-20">
            <div>
              <p className="text-xs font-semibold tracking-widest uppercase text-green mb-3">{T.contact.label}</p>
              <h2 className="font-serif text-4xl md:text-5xl text-charcoal mb-6">{T.contact.heading}</h2>
              <p className="text-charcoal/60 leading-relaxed mb-8">{T.contact.sub}</p>
              <div className="space-y-4 text-sm">
                <ContactRow icon={<IconPin />} label={T.contact.visitUs}>
                  Paseky 47, 756 57 Valašská Bystřice, Czech Republic
                </ContactRow>
                <ContactRow icon={<IconMail />} label={T.contact.email}>
                  <a href="mailto:info@freehorses.org" className="text-brown hover:underline">
                    info@freehorses.org
                  </a>
                </ContactRow>
                <ContactRow icon={<IconPhone />} label={T.contact.phone}>
                  <a href="tel:+420603123456" className="text-brown hover:underline">+420 603 123 456</a>
                </ContactRow>
                <ContactRow icon={<IconClock />} label={T.contact.hours}>
                  {T.contact.hoursVal}
                </ContactRow>
              </div>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-cream border border-charcoal/10 rounded-sm p-5">
                <p className="text-xs font-semibold tracking-widest uppercase text-charcoal/40 mb-2">{T.contact.bankTransfer}</p>
                <p className="text-sm text-charcoal/60 mb-1">Free Horses z.s.</p>
    ...
    >
      ...
      <div>
        <div className="flex items-start justify-between gap-1 mb-0.5">
          <h3 className="font-serif text-base md:text-sm text-charcoal leading-tight">{horse.name}</h3>
          <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-sm shrink-0 mt-0.5 leading-tight ${status.cls}`}>
            {statusLabel}
          </span>
        </div>
        <p className="text-[10px] text-charcoal/45 leading-tight">{horse.age}y · {horse.breed}</p>
      </div>
    </button>
  );
}
...
function HelpCard({
  ...
  const accentCls = { brown: 'text-brown bg-brown/8', green: 'text-green bg-green/8', sky: 'text-sky bg-sky/15' }[accent];
  const ctaCls = {
  ...
  return (
    <div className="border border-charcoal/10 rounded-sm p-7 bg-cream flex flex-col">
      <div className={`w-11 h-11 rounded-sm flex items-center justify-center mb-5 ${accentCls}`}>{icon}</div>
      ...
      <button onClick={onClick} className={`self-start text-sm font-semibold border px-4 py-2 rounded-sm transition-colors ${ctaCls}`}>
        {cta}
      </button>
    </div>
  );
...
function StoryCard({
  ...
  date: string; title: string; body: string; photo: string; alt: string; horseId: string;
  navigate: (p: string, id?: string) => void;
}) {
  return (
    <button onClick={() => navigate('horse', horseId)} className="group text-left flex flex-col">
      <div className="aspect-[16/11] bg-cream-dark overflow-hidden rounded-sm mb-5">
        <img src={photo} alt={alt} className="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-500" loading="lazy" />
      </div>
...
function IconHeart() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78z" /></svg>;
}
function IconHands() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 11V6a2 2 0 0 0-2-2 2 2 0 0 0-2 2" /><path d="M14 10V4a2 2 0 0 0-2-2 2 2 0 0 0-2 2v2" /><path d="M10 10.5V6a2 2 0 0 0-2-2 2 2 0 0 0-2 2v8" /><path d="M18 8a2 2 0 1 1 4 0v6a8 8 0 0 1-8 8h-2c-2.8 0-4.5-.86-5.99-2.34l-3.6-3.6a2 2 0 0 1 2.83-2.82L7 15" /></svg>;
}
function IconHorse() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3,18 C3,18 5,12 9,10 C11,9 13,10 15,8 C17,6 17,4 19,4 L21,4" /><path d="M9,10 L9,18" /><path d="M15,13 L15,18" /></svg>;
}
function IconDoc() {
  return <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="9" y1="13" x2="15" y2="13" /><line x1="9" y1="17" x2="13" y2="17" /></svg>;
}
function IconPin() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>;
}
function IconMail() {
...
function IconPhone() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13.5a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 2.7h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 10.1a16 16 0 0 0 6 6l1.46-1.46a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" /></svg>;
}
function IconClock() {
